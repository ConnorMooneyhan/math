...
\begin{proof}\
	\begin{enumerate}[label=(\alph*)]
		\item \textcolor{red}{UNFINISHED} Let $g, h \in G$, and choose $x \in G$ such that $xZ(G)$ generates $G/Z(G)$.
		      If either $g$ or $h$ is in $Z(G)$, then $gh = hg$ by definition, so consider when neither is in $Z(G)$.
		      Then $gZ(G) = x^mZ(G)$ and $hZ(G) = x^nZ(G)$ for some $m, n \in \mathbb{Z}^+$, so (letting $\overline{\cdot}$ denote $\cdot Z(G)$), \begin{equation*}
			      \overline{g}\overline{h} = \overline{x^m }\overline{x^n} = \overline{x^{m+n}} = \overline{x^{n + m }} = \overline{x^n}\overline{x^m } = \overline{h}\overline{g}
		      \end{equation*}
...
\begin{ex}
	...
	\begin{enumerate}[label=(\alph*)]
		...
		\item We now compute the stabilizer of an $H$-conjugacy class.
		      ...
		      Use this to conclude that $\mathrm{Stab}([x]_H) \subset HC_G(x)$.
