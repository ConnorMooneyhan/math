...
Disclaimer: Generative AI was used to help me understand how matrices of different types might act on $\mathbb{R}^n$ after I failed to be able to reason through it entirely alone (it's been a while since I did linear algebra and I couldn't find the relevant facts in my books).
\begin{ex}
...
\begin{proof}
	\begin{enumerate}[label=(\alph*)]
		\item Let $M, N$ be upper triangular matrices.
		      The inverse of an upper triangular matrix is itself upper triangular, so $MN^{-1}$ is the product of two upper triangular matrices, which is known to be upper triangular.
          Thus $MN^{-1} \in B_n(F)$.
		\item Indeed, if $M$ is a $n \times n$ matrix with determinant 1, then it is invertible since $1 \neq 0$, and \begin{align*}
			               & MM^{-1} = I_n                     \\
			      \implies & (\det M) (\det M^{-1}) = \det I_n \\
			      \implies & \det M^{-1} = 1.
		      \end{align*}
		      Then for any two $n \times n$ matrices $M$ and $N$ both having determinant 1, \begin{equation*}
			      \det(MN^{-1}) = (\det M) (\det N^{-1}) = 1,
		      \end{equation*}
          so $MN^{-1} \in \mathrm{SL}_n(F)$.
        \item By definition, the transpose of an orthogonal matrix is its inverse and is also orthogonal. Coupling this with the standard fact from linear algebra that the product of orthogonal matrices is orthogonal, we see that for $M, N \in O_n(F)$, $MN^{-1} \in O_n(F)$.
        \item As we have seen, a product of upper triangular matrices is upper triangular, and additionally, if there are 1s along the diagonal, both an upper triangular matrix and its inverse has 1s along the diagonal.
          Given any $M, N \in H_n(F)$, then, since their product will also have 1s along the diagonal, $MN^{-1} \in H_n(F)$.
	\end{enumerate}
\end{proof}
...
\begin{proof}
  ...
  Indeed, any nonzero vector can be transformed into any other by way of a nonsingular matrix, and the 0 vector can only be transformed to itself.

	For $\mathsf{SL}_n(\mathbb{R})$, we again have the orbits being $\mathbb{R}^n \setminus \left\lbrace 0 \right\rbrace$ and $\left\lbrace 0 \right\rbrace$.
  To show that we can take any nonzero vector $v$ to another nonzero vector $w$, we can extend a nonzero vector $v \in \mathbb{R}^n$ to a basis, then send the first basis to the second so that $v$ maps to $w$, then scale $z$ as needed so that the determinant of this matrix turns out to be 1.

  For $O_n(\mathbb{R})$, things start to get interesting.
  Here, lengths are preserved under multiplication by an orthogonal matrix, so the orbits end up being $\left\lbrace 0 \right\rbrace$ and the $(n-1)$-spheres of radius $r > 0$ about the origin.
\end{proof}
